...
def background_remover_raw():
    ###╔img = input_image('C:\\Users\\JQ\\Documents\\GitHub\\ABC\\CV_M1\\W1\\QSD2\\00005.jpg')
    #--------- Split planes and remove unnecessary information as possible --------
    rgb_planes = cv.split(img)
    result_planes = []
    result_norm_planes = []
    ...
    plt.imshow(cv.cvtColor(backtorgb, cv.COLOR_BGR2RGB))
    #---------------------------------------------------------- Save the mask and the image + mask
    directory = r'C:\\Users\\JQ\\Documents\\GitHub\\ABC\\CV_M1\\W1'
    os.chdir(directory)
    print("Before saving image:")  
    print(os.listdir(directory))  
    filename = 'mask.png'
    ...
    backtorgb = cv.imread('C:\\Users\\JQ\\Documents\\GitHub\\ABC\\CV_M1\\W1\\mask.png') #, cv.IMREAD_GRAYSCALE
    cv.floodFill(backtorgb, mask, (cx,cy), (0,255,0))
    added_image = cv.addWeighted(img,0.5,backtorgb,0.5,0)
