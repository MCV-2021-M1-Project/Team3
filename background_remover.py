    ...
    def compute_background(self, img):
        ...
        for cnt in contours[:MAX_N_FRAMES]:
            ...
            if max_area/ (img.shape[0] * img.shape[1]) > MIN_AREA_IMG_RATIO:
                ...
                angles.append([mod_angle])   
                #cv.drawContours(img_to_draw, cnt, -1, (0, 255, 0), 3)
                #x,y,w,h = cv.boundingRect(hull)
                resulting_frame_pos.append([x,y,w,h,mod_angle]) 
...
#        _,x,y,w,h,x2,y2,w2,h2 = museum.background_remover(load_directory + f,save_direcory,save_directory_croped ,file_name)
        ...
        frames.append(list_boxes_sorted)
        #print("Result")
        #print(result[idx])
        #print("Guess")
    ...
    ### Mean angular error
    #1. We obtain two list with only the angles
    angle_predicted = []
    angle_gt = []
    for p in frames:
        angle_predicted.append(p[0][0])  
    #print(angle_predicted)
    for q in result:
        angle_gt.append(q[0][0])
    #print(angle_gt)

    #2. Normalize angles into 0-90 degrees
    pred = []
    for angle in angle_predicted:
        if angle < 90:
            pred.append(angle)
        else: 
            pred.append(180-angle) 

    gt = []
    for angle in angle_gt:
        if angle < 90:
            gt.append(angle)
        else: 
            gt.append(180-angle)

    diff = [abs(x1 - x2) for (x1, x2) in zip(pred, gt)] #new list with angles differences
    mean_angular_error = sum(diff)/len(diff) #Mean angular error
